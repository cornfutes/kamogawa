...
CMD pg_ctl start; psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = 'shimogawa_db'" | grep -q 1 || psql -U postgres -c "CREATE DATABASE shimogawa_db"; psql -U postgres "CREATE EXTENSION pg_trgm;"; psql -U postgres "CREATE INDEX project_dbs_idx ON project_dbs USING GIN ((name || ' ' || project_id) gin_trgm_ops);"; psql -U postgres "CREATE INDEX gce_instance_dbs_idx ON gce_instance_dbs USING GIN ((id || ' ' || name || ' ' || status || ' ' || project_id || ' ' || zone) gin_trgm_ops);"; psql -U postgres "CREATE INDEX gae_service_dbs_idx ON gae_service_dbs USING GIN ((name || ' ' || id || ' ' || project_id) gin_trgm_ops);"; psql -U postgres "CREATE INDEX gae_version_dbs_idx ON gae_version_dbs USING GIN ((name || ' ' || id || ' ' || project_id || ' ' || service_name || ' ' || service_id || ' ' || serving_status) gin_trgm_ops) ;"; kamogawa-server;
